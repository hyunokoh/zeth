...
from zeth.core.constants import NOTE_LENGTH_BYTES, bit_length_to_byte_length
from zeth.core.utils import digest_to_binary_string
...
def _derive_a_pk(a_sk: EncryptionSecretKey) -> EncryptionPublicKey:
    """
    Returns a_pk = blake2s(1100 || [a_sk]_252 || 0^256)
    """
    binary_a_sk = digest_to_binary_string(encode_encryption_secret_key(a_sk))
    first_252bits_ask = binary_a_sk[:252]
    #left_leg_bin = "1100" + first_252bits_ask
    left_leg_bin = binary_a_sk
    ...
    a_pk = blake2s(
        ...
        encode_abi(["bytes32"], [encode_encryption_secret_key(a_sk)])
    ).digest()
    return decode_encryption_public_key(a_pk)

def _hash( b_pk: EncryptionPublicKey, u_pk: EncryptionPublicKey, h_pk: EncryptionPublicKey) -> EncryptionPublicKey:
    a_pk = blake2s(
        encode_abi(
            ["bytes32", "bytes32", "bytes32"],
            [encode_encryption_public_key(b_pk),encode_encryption_public_key(u_pk),encode_encryption_public_key(h_pk)])
    ).digest()
    return decode_encryption_public_key(a_pk)

def generate_zklay_encryption_keypair(audit_pk : AuditAddressPub) -> ZklayEncryptionKeyPair:
    sk = generate_encryption_secret_key()
    bk = _derive_a_pk(sk)
    hk = get_encryption_public_key(sk)
    uk_bytes =  _exchange(sk, audit_pk.addr_pk)
    uk = decode_encryption_public_key(uk_bytes)
    ak = _hash(bk, uk, hk)
    return ZklayEncryptionKeyPair(sk, ak, bk, uk, hk)
...
def decrypt_sym(
        ct_sym: bytes,
        addr_sk: EncryptionSecretKey) -> bytes:
    """
    Decrypts an encrypted message using a symmetry key cipher
    """
    sk_bytes = encode_abi(["bytes32"], [encode_encryption_secret_key(addr_sk)])
